...
ch_stg3 = cell(1,length(Selection));
ch_spindles_bnd = cell(1,length(Selection));
ch_spindles_quartile = cell(1,length(Selection));
ch_so_bnd = cell(1,length(Selection));
ch_so_quartile = cell(1,length(Selection));
...
for ch=1:length(Selection)%size(stg2segs,2)
    ...
    [can_ind_sw,~] = myso_det(ch_stg2{sg,ch},fs);
    if ~isempty(can_ind_sw)
        can_ind_sw = can_ind_sw + bnd(stg2segs(sg),1);
        ch_so = [ch_so;can_ind_sw(:,1) can_ind_sw(:,3)];
    end
    end
...
for ch=1:length(Selection)%size(stg3segs,2)
    ch_so  = ch_so_bnd{1,ch};
    for sg = 1:length(stg3segs)
    ch_stg3{sg,ch}=XEEG(ch,bnd(stg3segs(sg),1):bnd(stg3segs(sg),2));
    [can_ind_sw,~] = myso_det(ch_stg3{sg,ch},fs);
    if ~isempty(can_ind_sw)
        can_ind_sw = can_ind_sw + bnd(stg3segs(sg),1);
        ch_so = [ch_so;can_ind_sw(:,1) can_ind_sw(:,3)]
    end
    end
...
assignin('base','ch_spindles_bnd',ch_spindles_bnd);
assignin('base','sbj_hrb_ind2',sbj_hrb_ind2);
sbj_hrb_spindles2 = myHRB_spindle_intersect(sbj_hrb_ind2, ch_spindles_bnd, hrbwin,fs);
sbj_hrb_so2 = myHRB_spindle_intersect(sbj_hrb_ind2, ch_so_bnd, hrbwin,fs); % new change for SO
for c = 1:length(sbj_hrb_spindles2)
    sbj_hrb_spindles_densityPerMinute2{1,c} = mean(sbj_hrb_spindles2{1,c},1)*(60/avbin); 
end
for c = 1:length(sbj_hrb_so2)
    sbj_hrb_so_densityPerMinute2{1,c} = mean(sbj_hrb_so2{1,c},1)*(60/avbin); 
end
...
sbj_hrb_so3 = myHRB_spindle_intersect(sbj_hrb_ind3, ch_so_bnd, hrbwin,fs);% new change for SO
for c = 1:length(sbj_hrb_spindles3)
    sbj_hrb_spindles_densityPerMinute3{1,c} = mean(sbj_hrb_spindles3{1,c},1)*(60/avbin); 
end
for c = 1:length(sbj_hrb_so3)
    sbj_hrb_so_densityPerMinute3{1,c} = mean(sbj_hrb_so3{1,c},1)*(60/avbin); 
end
...
for ch=1:length(Selection)
    for ix = 1:size(ch_so_bnd{1,ch},1)
        if ch_so_bnd{1,ch}(ix,1)>=q1_s && ch_so_bnd{1,ch}(ix,1)<=q1_e
            ch_so_quartile{1,ch}(ix,1) = 1;
        elseif ch_so_bnd{1,ch}(ix,1)>=q2_s && ch_so_bnd{1,ch}(ix,1)<=q2_e
            ch_so_quartile{1,ch}(ix,1) = 2;
        elseif ch_so_bnd{1,ch}(ix,1)>=q3_s && ch_so_bnd{1,ch}(ix,1)<=q3_e
            ch_so_quartile{1,ch}(ix,1) = 3;
        elseif ch_so_bnd{1,ch}(ix,1)>=q4_s 
            ch_so_quartile{1,ch}(ix,1) = 4;
        elseif ch_so_bnd{1,ch}(ix,1)<q1_s 
            ch_so_quartile{1,ch}(ix,1) = 0;
        end   
    end
end
%%%%
...
for i=1:length(Selection)
    ...
    eval(['ace.binSO_Density_stg2_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute2{1,i};']);
    eval(['ace.binSO_Count_stg2_' Channels{1, Selection(i)} '= sbj_hrb_so2{1,i};']);
    eval(['ace.binSO_Density_sws_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute3{1,i};']);
    eval(['ace.binSO_Count_sws_' Channels{1, Selection(i)} '= sbj_hrb_so3{1,i};']);
end
...
bl_soDensity_2 = myblspindle(in2,bnd,sbj_hrb_ind2,hrbwin,fs, ch_so_bnd);
bl_soDensity_3 = myblspindle(in3,bnd,sbj_hrb_ind3,hrbwin,fs, ch_so_bnd);
...
%%%%%%%%
for ch = 1:length(Selection)
    ch_so_bnd_q1{1,ch} = ch_so_bnd{1,ch}(find(ch_so_quartile{1,ch}==1),:);
    ch_so_bnd_q2{1,ch} = ch_so_bnd{1,ch}(find(ch_so_quartile{1,ch}==2),:);
    ch_so_bnd_q3{1,ch} = ch_so_bnd{1,ch}(find(ch_so_quartile{1,ch}==3),:);
    ch_so_bnd_q4{1,ch} = ch_so_bnd{1,ch}(find(ch_so_quartile{1,ch}==4),:);
end
sbj_hrb_so2_q1 = myHRB_spindle_intersect(sbj_hrb_ind2, ch_so_bnd_q1, hrbwin,fs);
sbj_hrb_so2_q2 = myHRB_spindle_intersect(sbj_hrb_ind2, ch_so_bnd_q2, hrbwin,fs);
sbj_hrb_so2_q3 = myHRB_spindle_intersect(sbj_hrb_ind2, ch_so_bnd_q3, hrbwin,fs);
sbj_hrb_so2_q4 = myHRB_spindle_intersect(sbj_hrb_ind2, ch_so_bnd_q4, hrbwin,fs);

for c = 1:length(sbj_hrb_so2)
    sbj_hrb_so_densityPerMinute2_q1{1,c} = mean(sbj_hrb_so2_q1{1,c},1)*(60/avbin); 
    sbj_hrb_so_densityPerMinute2_q2{1,c} = mean(sbj_hrb_so2_q2{1,c},1)*(60/avbin); 
    sbj_hrb_so_densityPerMinute2_q3{1,c} = mean(sbj_hrb_so2_q3{1,c},1)*(60/avbin); 
    sbj_hrb_so_densityPerMinute2_q4{1,c} = mean(sbj_hrb_so2_q4{1,c},1)*(60/avbin); 
end

sbj_hrb_so3_q1 = myHRB_spindle_intersect(sbj_hrb_ind3, ch_so_bnd_q1, hrbwin,fs);
sbj_hrb_so3_q2 = myHRB_spindle_intersect(sbj_hrb_ind3, ch_so_bnd_q2, hrbwin,fs);
sbj_hrb_so3_q3 = myHRB_spindle_intersect(sbj_hrb_ind3, ch_so_bnd_q3, hrbwin,fs);
sbj_hrb_so3_q4 = myHRB_spindle_intersect(sbj_hrb_ind3, ch_so_bnd_q4, hrbwin,fs);

for c = 1:length(sbj_hrb_so3)
    sbj_hrb_so_densityPerMinute3_q1{1,c} = mean(sbj_hrb_so3_q1{1,c},1)*(60/avbin); 
    sbj_hrb_so_densityPerMinute3_q2{1,c} = mean(sbj_hrb_so3_q2{1,c},1)*(60/avbin); 
    sbj_hrb_so_densityPerMinute3_q3{1,c} = mean(sbj_hrb_so3_q3{1,c},1)*(60/avbin); 
    sbj_hrb_so_densityPerMinute3_q4{1,c} = mean(sbj_hrb_so3_q4{1,c},1)*(60/avbin); 
end

bl_soDensity_2_q1 = myblspindle(in2,bnd,sbj_hrb_ind2,hrbwin,fs, ch_so_bnd_q1);
bl_soDensity_2_q2 = myblspindle(in2,bnd,sbj_hrb_ind2,hrbwin,fs, ch_so_bnd_q2);
bl_soDensity_2_q3 = myblspindle(in2,bnd,sbj_hrb_ind2,hrbwin,fs, ch_so_bnd_q3);
bl_soDensity_2_q4 = myblspindle(in2,bnd,sbj_hrb_ind2,hrbwin,fs, ch_so_bnd_q4);

bl_soDensity_3_q1 = myblspindle(in3,bnd,sbj_hrb_ind3,hrbwin,fs, ch_so_bnd_q1);
bl_soDensity_3_q2 = myblspindle(in3,bnd,sbj_hrb_ind3,hrbwin,fs, ch_so_bnd_q2);
bl_soDensity_3_q3 = myblspindle(in3,bnd,sbj_hrb_ind3,hrbwin,fs, ch_so_bnd_q3);
bl_soDensity_3_q4 = myblspindle(in3,bnd,sbj_hrb_ind3,hrbwin,fs, ch_so_bnd_q4);
...
for i=1:length(Selection)
    ...
    eval(['ace.blSO_Density_stg2_' Channels{1, Selection(i)}  '= bl_soDensity_2(1,i);']);
    eval(['ace.blSO_Density_sws_' Channels{1, Selection(i)}  '= bl_soDensity_3(1,i);']);
    ...
    eval(['ace.bl_Q1_SO_Density_stg2_' Channels{1, Selection(i)}  '= bl_soDensity_2_q1(1,i);']);
    eval(['ace.bl_Q2_SO_Density_stg2_' Channels{1, Selection(i)}  '= bl_soDensity_2_q2(1,i);']);
    eval(['ace.bl_Q3_SO_Density_stg2_' Channels{1, Selection(i)}  '= bl_soDensity_2_q3(1,i);']);
    eval(['ace.bl_Q4_SO_Density_stg2_' Channels{1, Selection(i)}  '= bl_soDensity_2_q4(1,i);']);
    
    eval(['ace.bl_Q1_SO_Density_stg3_' Channels{1, Selection(i)}  '= bl_soDensity_3_q1(1,i);']);
    eval(['ace.bl_Q2_SO_Density_stg3_' Channels{1, Selection(i)}  '= bl_soDensity_3_q2(1,i);']);
    eval(['ace.bl_Q3_SO_Density_stg3_' Channels{1, Selection(i)}  '= bl_soDensity_3_q3(1,i);']);
    eval(['ace.bl_Q4_SO_Density_stg3_' Channels{1, Selection(i)}  '= bl_soDensity_3_q4(1,i);']);
    ...
    eval(['ace.binSO_Density_stg2_Q1_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute2_q1{1,i};']);
    eval(['ace.binSO_Density_stg2_Q2_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute2_q2{1,i};']);
    eval(['ace.binSO_Density_stg2_Q3_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute2_q3{1,i};']);
    eval(['ace.binSO_Density_stg2_Q4_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute2_q4{1,i};']);
    ...
    eval(['ace.binSO_Density_stg3_Q1_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute3_q1{1,i};']);
    eval(['ace.binSO_Density_stg3_Q2_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute3_q2{1,i};']);
    eval(['ace.binSO_Density_stg3_Q3_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute3_q3{1,i};']);
    eval(['ace.binSO_Density_stg3_Q4_' Channels{1, Selection(i)} '= sbj_hrb_so_densityPerMinute3_q4{1,i};']);
end
...
for i=1:length(Selection)
    ...
    save([PathName 'ace' FileName(1:end-4)],'ace', '-v7.3')
end
...
function [can_ind_sw,can_ind_dlta]=myso_det(x,fs)
...
cd=[can_ind(:,2)-can_ind(:,1) can_ind(:,3)-can_ind(:,2)]./fs.*1000;
